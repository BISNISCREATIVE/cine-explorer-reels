  ...
  return (
    <div className="relative group">
      <Link to={`/movie/${movie.id}`} className="group block hover:scale-[1.04] transition cursor-pointer">
        <div className={`relative overflow-hidden rounded-none bg-black shadow-lg border border-[#232831] ${compact ? '' : 'h-full'}`}>
          ...
          <div className="aspect-[2/3] relative">
            {!imageLoaded && (
              <div className="absolute inset-0 bg-gray-700 animate-pulse rounded-none" />
            )}
            <img
              src={tmdbApi.getImageUrl(movie.poster_path)}
              alt={movie.title}
              className={`w-full h-full object-cover rounded-none transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
              onLoad={() => setImageLoaded(true)}
            />
          </div>

          {/* Card Content */}
          <div className="p-3 flex flex-col flex-1 items-start gap-3">
